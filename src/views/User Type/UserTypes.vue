<template>
    <div class="col-lg-12 control-section">
        <div>
            <ejs-treegrid ref='treegrid' :toolbar="toolbar" :rowHeight='rowHeight'  :dataSource='data' childMapping='subgroups' :height='height' :allowReordering='true' :allowFiltering='true'
...
export default {
  ...
  async beforeMount() {
    ...
    for(i=0;i<this.data.length;i++) {
      api.get(`${apiUrl}`+`super/group/usertype/getall/`+`${data[i]._id}`).then((response)=> {
        console.log(response.data);
        this.data[i].subgroups = response.data
      })
